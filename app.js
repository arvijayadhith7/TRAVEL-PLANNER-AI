...
const TripPlanner = {
    ...
    async generateAIItinerary() {
        ...
        try {
            const response = await fetch('/api/ai/plan-itinerary', {
                method: 'POST',
                headers: { 'Content-Type': 'application/json' },
                body: JSON.stringify({ tripData, cart })
            });

            if (!response.ok) throw new Error('Failed to fetch AI itinerary');

            const itinerary = await response.json();
            this.saveAIItinerary(itinerary);
            return itinerary;
        } catch (error) {
    ...
    saveAIItinerary(itinerary) {
        localStorage.setItem('aiItinerary', JSON.stringify(itinerary));
    },

    getAIItinerary() {
        const data = localStorage.getItem('aiItinerary');
